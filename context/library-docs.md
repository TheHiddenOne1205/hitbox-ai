...
### Pitch Deck & GDD PDF Blueprint Generation
...
const styles = StyleSheet.create({
  page: { padding: 40, fontFamily: 'Helvetica', backgroundColor: '#121214' },
  section: { marginBottom: 14 },
  title: { fontSize: 18, color: '#ff4a5a', fontWeight: 'bold' },
  bodyText: { fontSize: 11, color: '#e1e1e6', lineHeight: 1.5 },
})

const GDDPitchPDF = ({ project }: { project: any }) => (
  <Document>
    <Page size="LETTER" style={styles.page}>
      <View style={styles.section}>
        <Text style={styles.title}>{project.title}</Text>
        <Text style={styles.bodyText}>{project.genre}</Text>
      </View>
    </Page>
  </Document>
)

// Generate dynamic byte array buffer inside Serverless endpoints
const buffer = await renderToBuffer(<GDDPitchPDF project={projectData} />)

// Upload binary data directly to InsForge Cloud Storage
await insforge.storage
  .from('drafts')
  .upload(`drafts/${userId}/${projectId}/gdd.pdf`, buffer, {
    contentType: 'application/pdf',
    upsert: true
  })
...
* Keep PDF generation logic entirely on the server — never import `@react-pdf/renderer` or compile components inside client-side modules.
* Always compile PDF structures using `renderToBuffer` to facilitate clean data uploads directly to your S3 buckets.
* Stick strictly to supported layout styles (like `flexDirection`, `justifyContent`, `fontSize`, `lineHeight`); unmapped layout style tags are ignored by the renderer engine.
